...
def test_resolve_training_model_uses_selected_rgb_input_channels(monkeypatch):
    ...
    assert captured == {
        'model_name': 'S 660k',
        'input_channels': 3,
        'kwargs': {'deep_supervision': True},
    }
    assert int(getattr(model, '_neuralimage_input_channels')) == 3
    assert save_path == Path('tmp_train') / 'S 660k_shift_32_epoch1.pth'


def test_resolve_training_model_enables_deep_supervision_for_loaded_conv_model(monkeypatch):
    import neuralimage.model.general_neural_handler as target
    loaded_model = SimpleNamespace(
        _neuralimage_input_channels=1,
        _neuralimage_model_kwargs={},
        deep_supervision=False,
    )
    checkpoint_path = Path('tmp_models') / 'conv_checkpoint.pth'
    ...
    assert loaded_model.deep_supervision is True
    assert loaded_model._neuralimage_model_kwargs['deep_supervision'] is True
