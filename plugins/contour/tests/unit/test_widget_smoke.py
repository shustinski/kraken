...
class WidgetSmokeTests(unittest.TestCase):
    ...
    def test_conductor_display_defaults(self) -> None:
        ...
        try:
            ...
            self.assertEqual(widget.metal_conductor_size_offset_slider.value(), 0)
            self.assertEqual(widget._current_contour_settings().metal_conductor_size_offset_px, 0.0)
            self.assertEqual(widget.metal_ws_smoothing_spin.value(), 1.2)
            self.assertEqual(widget.metal_ws_core_margin_spin.value(), 23.0)
            self.assertEqual(widget.metal_ws_groove_margin_spin.value(), 19.0)
            self.assertEqual(widget.metal_ws_rim_probe_spin.value(), 2)
            self.assertEqual(widget.metal_ws_seed_speckle_spin.value(), 1)
            self.assertEqual(widget.metal_ws_valley_span_spin.value(), 5)
            self.assertEqual(widget.metal_ws_valley_depth_spin.value(), 65.0)
            ...
            self.assertFalse(hasattr(widget, "metal_preprocessing_group"))
            ...
            self.assertEqual(widget.metal_watershed_group.title(), "Параметры водораздела")
            self.assertEqual(widget.metal_random_walker_group.title(), "Случайное блуждание")
            self.assertEqual(widget.metal_graph_cut_group.title(), "Графовый разрез")
            self.assertEqual(widget.metal_reconstruction_group.title(), "Морфологическая реконструкция")
