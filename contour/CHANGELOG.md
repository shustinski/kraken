...
- Decomposed the `PolygonExtractionWidget` god-class: extracted UI builders,
  ...
  `polygon_widget/ui/`. `widget.py` shrunk from ~7 100 to ~3 800 lines (~46 %
  reduction) while keeping a stable public API guarded by a smoke test and a
  golden snapshot.
- Split `polygon_widget/graphics_view.py` into a `polygon_widget/graphics/`
  package (`editor_scene`, `editor_view`, `tools`, `geometry`) with a backward
  compatible shim.
- Split `polygon_widget/application/use_cases/processing.py` into a package
