...
## Задания Kraken Agent

Лёгкий клиент отправляет управляемые задания распознавания Kraken Agent на сервер.
Адрес задаётся переменной окружения `NEURALIMAGE_SERVER_URL` или сохранённой настройкой
клиента. В полной сборке переменная `NEURALIMAGE_SERVER_URL` включает удалённое выполнение
Agent-заданий; без неё сохраняется локальный запуск. Возвращаемые маски сохраняют
контракт Kraken: одноканальный PNG со значениями 0 и 255. Повторный запуск одного
Agent-задания использует тот же идентификатор запроса на сервере.
