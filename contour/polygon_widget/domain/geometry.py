...
def compute_polygon_metrics(points: Iterable[Point]) -> tuple[float, float, tuple[int, int, int, int]]:
    ...
    if len(vertices) == 1:
        ...
        return 0.0, 0.0, (int(floor(x_coord)), int(floor(y_coord)), 1, 1)
    if len(vertices) == 2:
        (x0, y0), (x1, y1) = vertices
        perimeter = hypot(x1 - x0, y1 - y0) * 2.0
        x_min = int(floor(min(x0, x1)))
        y_min = int(floor(min(y0, y1)))
        x_max = int(ceil(max(x0, x1)))
        y_max = int(ceil(max(y0, y1)))
    ...
    x_min = int(floor(min(x_values)))
    y_min = int(floor(min(y_values)))
    x_max = int(ceil(max(x_values)))
    y_max = int(ceil(max(y_values)))
