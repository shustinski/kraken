...
from polygon_widget.utils import _imread_unicode_safe
...
class UtilsTests(unittest.TestCase):
    def test_imread_unicode_safe_skips_cv2_imread_for_non_ascii_paths(self) -> None:
        ...
        with (
            ...
            patch("polygon_widget.utils.cv2.imread") as cv2_imread,
            patch(
                "polygon_widget.utils.np.fromfile",
                return_value=np.array([1, 2, 3], dtype=np.uint8),
            ) as np_fromfile,
            patch("polygon_widget.utils.cv2.imdecode", return_value=decoded) as cv2_imdecode,
        ):
    ...
    def test_imread_unicode_safe_raises_before_cv2_imread_for_missing_file(self) -> None:
        ...
        with patch("polygon_widget.utils.cv2.imread") as cv2_imread:
