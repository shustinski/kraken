...
from model.NeuralNetwork.model_train_and_recognition import ModelTrainer, NeuralRecognizer
...
class GeneralNeuralHandler:
    ...
    def _resolve_training_model(self):
        if self.work_mode in (WorkMode.train_and_recognition, WorkMode.train_only):
            ...
            model = torch.load(self.recognition_parameters.model, weights_only=False)
    ...
    def _get_zipped_samples(self, image_folder, label_folder) -> tuple[list[tuple[Path, Path]], list[tuple[Path, Path]]] | tuple[list[tuple[Path, Path]], None]:
        ...
        zipped_images = list(zip(image_files, label_files))
    ...
    def _start_training(self, model, model_save_path: Path):
        resume_from_checkpoint = self.work_mode == WorkMode.futher_training
        self.current_thread = ModelTrainer(
            ...
            use_multi_gpu=self.tranining_parameters.use_multi_gpu,
    ...
    def _start_recognition(self):
        self.current_thread = NeuralRecognizer(
