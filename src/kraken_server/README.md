...
Журнал обращений задаётся в `server.toml`, поле `[server] log_level`. Значение применяется при старте.

| Уровень | Что пишется |
|---|---|
| `none` | журнал обращений пуст |
| `low` | создание проектов и учётных записей |
| `medium` | то же, плюс создание слоёв и загрузка данных |
| `high` | каждое обращение к серверу |

Собственный access-log Uvicorn выключен, чтобы обращения не дублировались. Ошибки запуска Uvicorn по-прежнему печатаются.
