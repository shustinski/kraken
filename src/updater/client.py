...
def load_update_client_config(
    *,
    app_id: str = "kraken",
    config_path: str | Path | None = None,
    env_prefix: str | None = None,
) -> UpdateClientConfig:
    prefix = _normalize_env_prefix(env_prefix or app_id)
    env_url = str(os.getenv(f"{prefix}_UPDATE_URL", "")).strip()
    if env_url:
        env_channel = normalize_update_channel(os.getenv(f"{prefix}_UPDATE_CHANNEL"))
        return UpdateClientConfig(manifest_urls=((env_channel, env_url),), default_channel=env_channel)

    resolved_config_path = _resolve_config_path(app_id, config_path)
    ...
    default_channel = normalize_update_channel(
        str(payload.get("default_channel", manifest_urls[0][0] if manifest_urls else "stable"))
    )
    return UpdateClientConfig(manifest_urls=tuple(manifest_urls), default_channel=default_channel)
...
def load_selected_update_channel(
    ...
    *,
    ...
    if available_channels:
        normalized_available = tuple(normalize_update_channel(channel) for channel in available_channels if str(channel).strip())
...
def load_last_notified_version(
    channel: str | None = None,
    *,
    settings_org: str = "Kraken",
    settings_app: str = "Updater",
...
def save_last_notified_version(
    version: str,
    channel: str | None = None,
    *,
    settings_org: str = "Kraken",
...
def download_update_installer(
    release_info: ReleaseInfo | UpdateInfo,
    *,
    app_id: str = "kraken",
) -> Path:
    if not release_info.download_url:
        raise ValueError("Update manifest does not contain download_url.")
    target_dir = get_update_staging_dir(app_id)
    target_dir.mkdir(parents=True, exist_ok=True)
    source = str(release_info.download_url).strip()
    target_path = target_dir / _resolve_installer_name(release_info, source, app_id=app_id)
...
def get_update_staging_dir(app_id: str = "kraken") -> Path:
...
def collect_release_history(update_info: UpdateInfo) -> str:
    ...
    for release in relevant_entries:
        ...
        chunks.append(f"### {version_label}\n\n{notes}" if notes else f"### {version_label}")
    return "\n\n".join(chunks)
...
def _last_notified_key(channel: str | None = None) -> str:
    ...
    return f"{_UPDATE_SETTINGS_KEY}_{normalize_update_channel(channel)}"


def _parse_release_entries(raw_value: Any, *, source: str = "", default_channel: str = "stable") -> tuple[ReleaseInfo, ...]:
...
def _resolve_release_notes(payload: dict[str, Any], *, source: str = "") -> str:
    inline_markdown = str(
        payload.get("release_notes_markdown", payload.get("notes_markdown", payload.get("changes_markdown", ""))) or ""
    ).strip()
    if inline_markdown:
...
def _resolve_installer_name(update_info: ReleaseInfo | UpdateInfo, source: str = "", *, app_id: str = "kraken") -> str:
    source_name = _extract_source_filename(source)
    if source_name:
        return source_name
    version = _sanitize_filename(update_info.version.strip() or "latest")
...
def _resolve_config_path(app_id: str, config_path: str | Path | None) -> Path | None:
    ...
    candidates = [
        Path.cwd() / "resources" / "update_client.json",
        Path.cwd() / "plugins" / app_id / "resources" / "update_client.json",
    ]
    here = Path(__file__).resolve()
    for parent in here.parents:
        candidates.append(parent / "plugins" / app_id / "resources" / "update_client.json")
...
def _normalize_env_prefix(value: str) -> str:
    return re.sub(r"[^0-9A-Za-z]+", "_", str(value or "kraken")).strip("_").upper() or "KRAKEN"
