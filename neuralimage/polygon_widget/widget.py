...
from PyQt6.QtCore import QObject, QPointF, QRectF, QSize, Qt, QSettings, QRunnable, QThreadPool, QTimer, pyqtSignal
...
from .batch_processor import BatchProcessor, process_image_path
...
from .models import ContourExtractionSettings, DisplaySettings, ImageProcessingState, PolygonData, SaveOptions
...
from .utils import cv_to_qimage, is_image_path, load_image_grayscale, scan_image_files


def _polygon_widget_qsettings() -> QSettings:
    root = os.getenv("NEURALIMAGE_SETTINGS_DIR")
    if root:
        settings_root = Path(root)
        settings_root.mkdir(parents=True, exist_ok=True)
        return QSettings(
            str(settings_root / "NeuralImage_PolygonWidget.ini"),
            QSettings.Format.IniFormat,
        )
    return QSettings("NeuralImage", "PolygonWidget")
...
class PreparedImageRunnable(QRunnable):
    ...
    def run(self) -> None:
        try:
            preprocessed_image = PreprocessingPipeline.from_dict(self.pipeline_config).apply(self.source_image)
            self.signals.result.emit(self.request_id, self.image_path, preprocessed_image, self.pipeline_config)
        except Exception as exc:
            self.signals.error.emit(self.request_id, str(exc))
        finally:
            self.signals.finished.emit(self.request_id)
...
class PolygonExtractionWidget(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._preview_pending_request: dict[str, object] | None = None
        ...
        self._prepared_image_pending_request: dict[str, object] | None = None
    ...
    def _restore_persisted_paths(self) -> None:
        settings = _polygon_widget_qsettings()
        ...
        if cif_directory:
            self.set_cif_directory(cif_directory)
        if input_directory:
            self.set_input_directory(input_directory)

    def _save_persisted_paths(self) -> None:
        settings = _polygon_widget_qsettings()
        settings.setValue("paths/input_directory", self.input_dir_edit.text().strip())
        settings.setValue("paths/cif_directory", self.cif_dir_edit.text().strip())
        settings.setValue("paths/output_directory", self.output_dir_edit.text().strip())
        settings.sync()
    ...
    def _update_step_parameter(self, row: int, parameter_name: str, value) -> None:
        ...
        if self.auto_apply_checkbox.isChecked() and self._current_image_path:
    ...
    def _apply_cif_directory_edit(self) -> None:
        ...
        if path:
            ...
            self._cif_paths_by_stem = {}
            self._state_cache.clear()
            self._save_persisted_paths()
            if self._current_image_path:
                try:
                    self.load_image(self._current_image_path)
                except Exception as exc:
    ...
    def _sync_current_state_views(self) -> None:
        ...
        try:
            ...
            polygons = self._current_state.polygons if self._current_state else []
    ...
    def _display_image_for_current_state(self):
        if self._current_state is None:
            return None
        return self._current_state.preprocessed_image if self._current_state.preprocessed_image is not None else self._current_state.source_image
    ...
    def _build_preview_request(self) -> dict[str, object] | None:
    ...
    def _preview_request_signature(self, request: dict[str, object]) -> tuple[str, str, str]:
        contour_settings = request["contour_settings"]
        contour_payload = contour_settings.to_dict() if hasattr(contour_settings, "to_dict") else contour_settings
        return (
            str(request["image_path"]),
            json.dumps(request["pipeline_config"], ensure_ascii=False, sort_keys=True),
            json.dumps(contour_payload, ensure_ascii=False, sort_keys=True),
        )

    def _prepared_image_request_signature(self, request: dict[str, object]) -> tuple[str, str]:
        return (
            str(request["image_path"]),
            json.dumps(request["pipeline_config"], ensure_ascii=False, sort_keys=True),
        )
    ...
    def _on_prepared_image_result(self, request_id: int, image_path: str, preprocessed_image, pipeline_config: dict) -> None:
        ...
        if self._current_image_path == image_path and self._current_state is state:
            self._sync_current_state_views()
    ...
    def _on_polygons_edited(self) -> None:
        ...
        if self._current_state is None or self._current_image_path is None:
            return
        self._current_state.polygons = self.get_polygons()
    ...
    def set_input_directory(self, path: str) -> None:
        ...
        self._save_persisted_paths()
        self.load_images(scan_image_files(path))

    def set_cif_directory(self, path: str) -> None:
        normalized = str(Path(path))
        self.cif_dir_edit.setText(normalized)
        self._save_persisted_paths()
        cif_root = Path(normalized)
        self._cif_paths_by_stem = {}
        if cif_root.exists() and cif_root.is_dir():
            self._cif_paths_by_stem = {
                cif_path.stem.lower(): str(cif_path)
                for cif_path in sorted(cif_root.iterdir(), key=lambda item: item.name.lower())
                if cif_path.is_file() and cif_path.suffix.lower() == ".cif"
            }
            self._append_log(self._tr("cif_indexed_log", count=len(self._cif_paths_by_stem)))
        else:
    ...
    def load_images(self, paths: list[str]) -> None:
        self._image_paths = [str(Path(path)) for path in paths if is_image_path(path)]
        self._preview_update_timer.stop()
        self._preview_pending_request = None
        self._preview_pending_signature = None
        self._prepared_image_pending_request = None
    ...
    def _find_matching_cif_path(self, image_path: str) -> str | None:
        if not self._cif_paths_by_stem:
            return None
        stem = Path(image_path).stem.lower()
        return self._cif_paths_by_stem.get(stem)
    ...
    def load_image(self, path: str) -> None:
        ...
        if image_path in self._state_cache:
            ...
            self._current_state = self._state_cache[image_path]
            self._sync_current_state_views()
            if self._current_state.preprocessed_image is None and self._current_state.source_image is not None:
                self._queue_prepared_image_update(image_path, self._current_state.source_image)
            self._append_log(self._tr("loaded_cached_state_log", image_path=image_path))
            return
        source = load_image_grayscale(image_path)
        polygons = self._load_cif_overlay_polygons(image_path)
        self._current_image_path = image_path
        self._current_state = ImageProcessingState(
            image_path=image_path,
            source_image=source,
            polygons=polygons,
        )
        self._state_cache[image_path] = self._current_state
        self._sync_current_state_views()
        self._queue_prepared_image_update(image_path, source)
        self._append_log(self._tr("loaded_image_log", image_path=image_path))
    ...
    def save_current_result(
        ...
        saved_files = save_result_bundle(
            ...
            source_image=self._current_state.source_image,
    ...
    def start_batch_processing(
        ...
        if self._batch_processor.is_running:
            ...
            return
        paths = image_paths or list(self._image_paths)
        if not paths:
