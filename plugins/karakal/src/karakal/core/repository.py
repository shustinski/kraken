...
def _model_output_confidence_metrics(confidence_map: np.ndarray) -> ModelOutputConfidenceMetrics:
    ...
    score, mean_uncertainty, uncertain_fraction, top_uncertainty_mean, largest_region = _frame_uncertainty_components_from_maps(
        uncertainty,
        support_weights,
        uncertainty_threshold=float(MODEL_RISK_UNCERTAINTY_THRESHOLD),
        top_fraction=float(MODEL_RISK_TOP_UNCERTAIN_FRACTION),
    ...
    *,
    ...
    for spec in model_specs:
        ...
        if geometry_mode == GeometryMode.AUTO and first_point_candidate is None:
            first_point_candidate = build_prediction_view_from_gray(spec.display_name, np.asarray(mask_gray, dtype=np.uint8), threshold=int(round(float(spec.threshold) * 255.0)))
    ...
    for spec, mask_gray, prob_gray, uses_binary_probability_proxy in loaded_rows:
        ...
        if resolved_geometry_mode == GeometryMode.POINT:
            ...
            if need_prediction_view:
                prediction_view = build_prediction_view_from_gray(spec.display_name, np.asarray(mask_gray, dtype=np.uint8), threshold=int(round(float(spec.threshold) * 255.0)))
...
def _metric_requires_pairwise_metrics(metric_key: str | None) -> bool:
    ...
    if parsed is not None:
        family, _model_id = parsed
        return family not in {'model_confidence', 'model_uncertain_fraction', 'model_point_contrast', 'model_output_confidence'}
...
def _analyze_record_payload(
    ...
    include_model_diagnostics = bool(
        ...
        or (include_pairwise_metrics and geometry_mode != GeometryMode.MASK)
    )

    load_started = perf_counter()
    probabilities_by_model, output_probabilities_by_model, masks_by_model, _source_grays_by_model, model_structures, model_diagnostics, model_metrics, model_confidence, model_confidence_output_available, _original_gray, gt_mask, _gt_structure, resolved_geometry_mode, model_views, gt_point_view = _build_model_payloads(
        record,
        model_specs,
        analysis_max_side=analysis_max_side,
...
def _available_metric_keys_for_models(model_specs: tuple[ModelSpec, ...], records: tuple[FrameRecord, ...] | list[FrameRecord] | None = None) -> tuple[str, ...]:
    ...
    for spec in model_specs:
        ...
        if spec.prob_folder is not None and (model_ids_with_output is None or str(spec.model_id) in model_ids_with_output):
...
def export_record_assets(
    ...
    *,
    ...
    def copy_source(source_path_text: str | None, source_folder: Path | None, fallback_folder_name: str, role: str) -> None:
    ...
    gt_folder = build_result.gt_folder.path if build_result.gt_folder is not None else None
    copy_source(record.original_path or record.base_path, original_folder, "original", "original")
    copy_source(record.gt_path, gt_folder, "ground_truth", "ground_truth")
...
def compute_build_result_analytics(build_result: BuildResult, *, metric_key: str | None = None, progress_callback=None, state_callback=None, cancel_check=None) -> BuildResult:
    ...
    try:
        ...
        for record_key, payload in payload_iter:
            ...
            for model_id, confidence_output_row in model_confidence_output.items():
                metric_values[_model_metric_key('model_output_confidence', str(model_id))] = float(getattr(confidence_output_row, 'frame_uncertainty_score', 0.0))
    ...
    metric_is_required = _metric_requires_model_confidence(active_metric) or _metric_requires_model_output_confidence(active_metric)
...
def load_frame_detail_base(
    ...
    *,
    ...
    probabilities, output_probabilities, masks, model_source_grays, _model_structures, model_diagnostics, model_metrics, model_confidence, model_confidence_output_available, original_gray, gt_mask, _gt_structure, detail_geometry_mode, model_views, gt_point_view = _build_model_payloads(
        active_record,
    ...
    model_output_display_names = {
        spec.model_id: str(spec.prob_folder.name if spec.prob_folder is not None and spec.prob_folder.name else spec.display_name)
