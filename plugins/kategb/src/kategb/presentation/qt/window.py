...
_HELP_TEXT = (
    ...
    "2. Выберите исполнителей и сгенерируйте кадры для проверки.\n"
    "3. Сохраните зашифрованный файл проверки и скопируйте исходные CIF/JPG.\n"
    "4. После проверки загрузите файл проверки и XML-результат, чтобы посчитать ошибки по исполнителям."
)
...
class KateGBWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.tabs.addTab(self._build_sample_tab(), "Выборка")
        self.tabs.addTab(self._build_verification_tab(), "Проверка")
    ...
    def _build_sample_tab(self) -> QWidget:
        ...
        source_box = QGroupBox("Исходные данные")
        source_layout = QGridLayout(source_box)
        self.markup_path = QLineEdit()
        self.markup_path.setPlaceholderText("Файл разметки Excel (.xlsx)")
        self.load_markup_button = QPushButton("Загрузить разметку")
        self.layer_name = QLineEdit()
        self.layer_name.setPlaceholderText("Название слоя")
        self.authors = QLineEdit()
        self.authors.setPlaceholderText("Исполнители через запятую")
        self.cif_folder = QLineEdit()
        self.cif_folder.setPlaceholderText("Папка исходных CIF")
        self.jpg_folder = QLineEdit()
        self.jpg_folder.setPlaceholderText("Дополнительная папка исходных JPG")
        for row, (label, widget) in enumerate(
            (
                ("Разметка", self.markup_path),
                ("Слой", self.layer_name),
                ("Исполнители", self.authors),
                ("Папка CIF", self.cif_folder),
                ("Папка JPG", self.jpg_folder),
            )
        ):
        ...
        settings_box = QGroupBox("Параметры выборки")
        settings_layout = QGridLayout(settings_box)
        self.percent = QSpinBox()
        self.percent.setRange(1, 100)
        self.percent.setValue(100)
        self.percent.setToolTip("Сколько процентов кадров взять у каждого выбранного исполнителя.")
        self.frame_range = QLineEdit()
        self.frame_range.setPlaceholderText("Необязательно, например 10-250")
        ...
        self.mode_all = QRadioButton("Весь диапазон")
        self.mode_area = QRadioButton("Прямоугольная область")
        ...
        for row, (label, widget) in enumerate(
            (
                ("Процент на исполнителя", self.percent),
                ("Диапазон кадров", self.frame_range),
                ("Кадров в слое", self.frames_in_layer),
                ("Кадров в строке", self.frames_in_row),
            )
        ):
            settings_layout.addWidget(QLabel(label), row, 0)
            settings_layout.addWidget(widget, row, 1)
        settings_layout.addWidget(QLabel("Режим диапазона"), 4, 0)
        settings_layout.addLayout(mode_layout, 4, 1)
        grid.addWidget(settings_box, 1, 0, 1, 2)

        output_box = QGroupBox("Файл проверки и копирование")
        output_layout = QGridLayout(output_box)
        self.check_name = QLineEdit("proverka")
        self.encryption_key = QLineEdit()
        self.encryption_key.setPlaceholderText("Обязательный ключ шифрования")
        self.output_folder = QLineEdit()
        self.output_folder.setPlaceholderText("Папка для файла проверки и скопированных файлов")
        self.select_output_button = QPushButton("Выбрать")
        self.generate_button = QPushButton("Сгенерировать кадры")
        self.save_manifest_button = QPushButton("Сохранить файл проверки")
        self.copy_button = QPushButton("Скопировать выбранные файлы")
        self.copy_button.setEnabled(False)
        for row, (label, widget) in enumerate(
            (("Имя проверки", self.check_name), ("Ключ", self.encryption_key), ("Папка результата", self.output_folder))
        ):
    ...
    def _build_verification_tab(self) -> QWidget:
        ...
        self.select_manifest_button = QPushButton("Выбрать")
        self.select_xml_button = QPushButton("Выбрать")
        self.select_verify_markup_button = QPushButton("Выбрать")
        self.analyze_button = QPushButton("Рассчитать")
        for row, (label, widget, button) in enumerate(
            (
                ("Файл проверки", self.verify_manifest, self.select_manifest_button),
                ("XML результата", self.verify_xml, self.select_xml_button),
                ("Ключ", self.verify_key, None),
                ("Разметка", self.verify_markup, self.select_verify_markup_button),
            )
        ):
        ...
        self.results_table.setHorizontalHeaderLabels(("Исполнитель", "Проверено", "Ошибочные", "Ошибки, %"))
    ...
    def _connect_signals(self) -> None:
        ...
        self.select_manifest_button.clicked.connect(
            lambda: self._select_file(self.verify_manifest, "Файлы проверки (*.txt)")
        )
    ...
    def _load_demo_layer(self) -> None:
        layer = LayerInfo(
            name="Слой 1",
            author_frames={"Исполнитель A": tuple(range(1, 11)), "Исполнитель B": tuple(range(11, 21))},
            frames_in_layer=20,
            frames_in_row=10,
        )
    ...
    def _load_markup(self) -> None:
        ...
        try:
            ...
            self.status.setText(f"Загружено слоев: {len(self._crystal_info.layers)}.")
    ...
    def _generate_frames(self) -> None:
        try:
            ...
            self.status.setText(f"Сгенерировано кадров: {len(self._generated_frames)}.")
        except Exception as exc:
            self._show_error(str(exc))
        ...
        try:
            path = self._save_manifest.execute(
                GenerateManifestRequest(
                    ...
                    layer_name=self.layer_name.text().strip(),
                    check_name=self.check_name.text().strip() or "proverka",
                    frame_range_text=self.frame_range.text().strip(),
                    selection_mode="all" if self.mode_all.isChecked() else "area",
            ...
            self.status.setText(f"Файл проверки сохранен: {path}")
        except Exception as exc:
            self._show_error(str(exc))
    ...
    def _copy_progress(self, done: int, total: int, path: str) -> None:
        ...
        self.status.setText(f"Копирование: {path}")

    def _copy_finished(self, report: CopyReport) -> None:
        self.status.setText(f"Скопировано файлов: {len(report.copied_files)}. Пропущенные кадры: {list(report.missing_frames)}")
    ...
    def _manual_author_frames(self) -> dict[str, tuple[int, ...]]:
        authors = self._selected_authors()
        if not authors:
            raise ValueError("Введите хотя бы одного исполнителя.")
    ...
    def _select_file(self, line_edit: QLineEdit, file_filter: str) -> str:
        path, _ = QFileDialog.getOpenFileName(self, "Выберите файл", line_edit.text(), file_filter)
        if path:
